...
export interface SyncApi {
  ...
  // P3 #10: optional cancelToken lets the renderer abort an in-flight
  // prefetch run when it switches accounts mid-fetch. The shim forwards
  // it to the sidecar so the worker can drop unstarted batches.
  prefetchBodies: (
    ids: string[],
    cancelToken?: string,
  ) => Promise<IpcResponse<R<"sync.prefetchBodies">>>;
  prefetchBodiesCancel: (
    cancelToken: string,
  ) => Promise<IpcResponse<R<"sync.prefetchBodiesCancel">>>;
...
// ─────────────────────────────────────────────────────────────────────────
// Morning Briefing — daily Claude-generated digest.
// ─────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────
// Permissions — agent activity awaiting approval.
// ─────────────────────────────────────────────────────────────────────────

export interface PermissionsApi {
  list: (accountId?: string) => Promise<IpcResponse<R<"permissions.list">>>;
}
